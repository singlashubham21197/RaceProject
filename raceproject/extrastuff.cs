...
namespace raceproject
{
    partial class MainForm
    {
        ...
        {
            if (!horseSelectedTrueOrFalse() && bettorSelectedTrueOrFalse())
            {
                MessageBox.Show("No horse selected, cannot bet!");
            }
            else if (horseSelectedTrueOrFalse() && !bettorSelectedTrueOrFalse())
            {
        ...
        public bool horseSelectedTrueOrFalse() // is a horse selected
        {
        ...
        public void processBet(int i) // check for nonsensical conditions, e.g. betting more money than the player has available
        {
            ...
            else
            {
                ...
                myguy[i].dog = horseSelected; // assign the greyhound for this player
            }
        }
        ...
        public async void beginRace() // dangerously ugly code
        {
            ...
            while (victory == false)
            {
                for (int j = 0; j < 4; ++j) // for each horse
                {
